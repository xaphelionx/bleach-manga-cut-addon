...
The registry is append-only. `published` means an ID belongs to a currently published entry; `reserved` means only that the assignment cannot be reused. Registry presence never implies publication. All 103 default-projection IDs, Chipped #01 through #12 included, are now published and, as of this checkpoint, all 103 are additionally compatibility-locked after explicit validation decisions. All remaining registry assignments outside the default projection (HB51–62, CB52+, HB11.5, and EX entries) remain reserved and unlocked.

## Compatibility-lock baseline

The compatibility-locked prefix is now the complete explicit 103 IDs through Chipped #12 — the entire published default projection. This is a sequence of deliberate validation decisions, not a consequence of publication. CB1–CB3 retain stronger individually established historical fixtures. The first 37 entries retain their prior preboundary validation history; CB36–CB51 retain representative Arrancar validation; HB14 retains its separate complete proof; the remaining 37 Hollowed entries are accepted as a representative batch under the technical and real-device basis below; Chipped #01–#02 retain their separate owner-confirmed compatibility decisions; and Chipped #03–#12 are now locked from their own representative batch validation.
...
Current publication is the complete 103-entry canonical default projection, through Chipped #12 at S4E12/global index 103; there is no remaining unpublished default entry, and the generic `unpublished-primary` suffix gate is currently unused. All 103 entries, including Chipped #01–#12, are now compatibility-locked. Concentrated CB52+ and Hollowed 51–62 remain reserved source identities outside the default projection, not the next selected default Watch Guide entries. The generic suffix blocker definition is now `primary-publication-prefix-after-ch_12`; prefix closure continues to prevent gaps without a special project barrier.
...
- The current 103-entry published prefix is now also the complete 103-entry compatibility-locked prefix, through the separate Chipped #01–#02 individual and Chipped #03–#12 representative-batch validation decisions. Publication never grants a lock automatically.
