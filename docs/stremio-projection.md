...
The current eligible and published prefix contains 93 entries: all 53 default Concentrated entries through CB51, including CB27.5 and guided 35.5 / `cb_0p0`, followed by all 38 owner-selected current-raw Hollowed entries through HB50, then Chipped #01–#02 / `ch_1`–`ch_2` as a two-episode compatibility POC extension. The separate compatibility-locked prefix remains the first 92 entries through Chipped #01; Chipped #02 is published and `locked:false` pending real-device validation. The projection has 93 eligible and 10 blocked entries. Every remaining primary entry, beginning with Chipped #03, uses the generic `unpublished-primary` gate until publication is explicitly approved and the contiguous prefix advances. The final 37 Hollowed locks record a separate representative validation decision rather than following automatically from publication.
...
The registry is append-only. `published` means an ID belongs to a currently published entry; `reserved` means only that the assignment cannot be reused. Registry presence never implies publication. All 92 IDs through Chipped #01 are published and compatibility-locked after explicit validation decisions. Chipped #02 is the only later published assignment and remains unlocked; all assignments from Chipped #03 onward remain reserved and unlocked.

## Compatibility-lock baseline

The compatibility-locked prefix is now the explicit 92 IDs through Chipped #01. This is a sequence of deliberate validation decisions, not a consequence of publication. CB1–CB3 retain stronger individually established historical fixtures. The first 37 entries retain their prior preboundary validation history; CB36–CB51 retain representative Arrancar validation; HB14 retains its separate complete proof; the remaining 37 Hollowed entries are accepted as a representative batch under the technical and real-device basis below; and Chipped #01 retains its separate owner-confirmed compatibility decision.
...
- The current 93-entry published prefix extends one entry past the explicit 92-entry compatibility-locked prefix: Chipped #02 remains unlocked pending separate validation, demonstrating that publication never grants a lock automatically.
