...
The current eligible and published prefix now contains the complete 103-entry default projection: all 53 default Concentrated entries through CB51, including CB27.5 and guided 35.5 / `cb_0p0`, followed by all 38 owner-selected current-raw Hollowed entries through HB50, then all twelve selected Chipped entries, #01–#12 / `ch_1`–`ch_12`. The projection has 103 eligible and 0 blocked entries; there is no remaining unpublished default entry, and the generic `unpublished-primary` gate is currently unused by any entry (its definition remains available for a future extension beyond the current 103-record editorial model). Compatibility lock is a separate, narrower decision: only the 93-entry prefix through Chipped #02 is locked. Chipped #02 is locked after owner-confirmed compatibility validation; Chipped #03–#12 are published but remain unlocked pending their own representative real-device validation. The final 37 Hollowed locks record a separate representative validation decision rather than following automatically from publication.
...
The registry is append-only. `published` means an ID belongs to a currently published entry; `reserved` means only that the assignment cannot be reused. Registry presence never implies publication. All 103 default-projection IDs, Chipped #01 through #12 included, are now published. Of those, only the 93 IDs through Chipped #02 are additionally compatibility-locked after explicit validation decisions; Chipped #03 through #12 are published with `locked:false`, pending their own validation. All remaining registry assignments outside the default projection (HB51–62, CB52+, HB11.5, and EX entries) remain reserved and unlocked.

## Compatibility-lock baseline

The compatibility-locked prefix is now the explicit 93 IDs through Chipped #02 — a subset of the now fully published 103-entry default projection. This is a sequence of deliberate validation decisions, not a consequence of publication. CB1–CB3 retain stronger individually established historical fixtures. The first 37 entries retain their prior preboundary validation history; CB36–CB51 retain representative Arrancar validation; HB14 retains its separate complete proof; the remaining 37 Hollowed entries are accepted as a representative batch under the technical and real-device basis below; and Chipped #01–#02 retain their separate owner-confirmed compatibility decisions. Chipped #03–#12 are published but not yet part of this locked baseline.
...
Current publication is the complete 103-entry canonical default projection, through Chipped #12 at S4E12/global index 103; there is no remaining unpublished default entry, and the generic `unpublished-primary` suffix gate is currently unused. Chipped #01–#02 remain compatibility-locked after owner-confirmed validation; Chipped #03–#12 are published but unlocked, pending their own representative real-device validation. Concentrated CB52+ and Hollowed 51–62 remain reserved source identities outside the default projection, not the next selected default Watch Guide entries. The generic suffix blocker definition is now `primary-publication-prefix-after-ch_12`; prefix closure continues to prevent gaps without a special project barrier.
...
The full 12-entry Chipped span has separate technical evidence at `evidence/acquisition/chipped-selected.json` and in 12 `evidence/media/ch_*.json` records. On 2026-08-24, the project owner confirmed all twelve selected Chipped torrents are Download Ready in the owner's normal TorBox environment; this is a current-environment availability observation only, is not converted into editorial evidence, does not claim permanent future cache or swarm availability, and records no private TorBox identifier, URL, token, UUID, or account data. Chipped #01–#02 were published and separately compatibility-locked in prior checkpoints. Chipped #03–#12 were published together as a batch on the same date, completing the default 103-entry projection; they generate ordinary meta, stream, and provenance output from their evidence but are not compatibility-locked. Planned representative real-device validation cases are CH6 (the known 29.872-second discrepancy between the authoritative editorial runtime and the measured container duration), CH10 (its third audio track's unresolved language and Nuvio's audio-track presentation/selection behavior), and CH12 (terminal completion/no-next-episode behavior). These validations have not yet occurred.
...
- The projection contains 103 eligible and 0 blocked entries; the complete default projection is published, and the generic `unpublished-primary` suffix gate is currently unused by any entry.
- The current 103-entry published prefix retains a 93-entry compatibility-locked prefix through the separate Chipped #02 validation decision; Chipped #03–#12 are published but `locked:false`. Publication never grants a lock automatically.
