...
test('current 93-entry prefix preserves the original locked prefix and Chipped 02 boundary', () => {
  ...
  assert.equal(meta.meta.videos.filter((video) => video.season === 4).length, 2)
  assert.equal(meta.meta.videos.some((video) => video.id === 'ch_3' || video.id === 'cb_52'), false)
