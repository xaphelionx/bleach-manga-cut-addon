...
This Stremio addon publishes the current 54-entry default prefix: 53 Concentrated Bleach entries from CB1 through CB51, including CB27.5 and guided CB35.5, followed by Hollowed 14 as a compatibility proof. The 53 Concentrated entries remain compatibility-locked as the accepted batch baseline after deterministic technical validation and representative real-device sampling; HB14 is published but remains `locked:false` until separate real-device validation. The first 37 entries retain their prior preboundary validation history; CB36–CB51 extend the baseline through the documented representative Arrancar validation against Nuvio 0.8.4-beta with its native TorBox integration. **Concentrated Bleach 01 — Death and Strawberry**, **Concentrated Bleach 02 — Starter**, and **Concentrated Bleach 03 — The Pink-Cheeked Cockatiel** retain their stronger individually established historical fixtures. The batch lock does not imply that every episode was manually played or that every client UI behavior is bug-free. This is not a full catalog and is not deployed.

Current publication and validation state:

- **Published:** exactly 54 default entries: 53 Concentrated entries through `cb_51`, followed by Hollowed 14 (`hb_14`) at S3E17/global index 54. Guided CB35.5 is spreadsheet record `concentrated:0.0`, internal ID `cb_0p0`, and the final Season 2 episode at S2E28; CB36–CB51 occupy S3E1–S3E16.
- **Compatibility-locked batch:** the 53 Concentrated IDs through `cb_51` remain `published` and `locked:true`. HB14 is `published` and `locked:false`; publication does not grant a compatibility lock.
...
- **Canonical prefix stop:** Hollowed 15 (`hb_15`) is the first unpublished default entry at S3E18/global index 55. Concentrated CB52+ are reserved source identities, not the next selected Watch Guide entries.
...
- **Hollowed technical evidence:** the owner-selected 38-entry current raw membership has a separate deterministic acquisition manifest and 38 verified-media records. All 38 local trackerless torrents were independently reproduced byte-for-byte and all 33,764 pieces verified against the selected MP4 payloads. The project owner separately confirmed 38/38 are 100% and seeding in qBittorrent and 38/38 are Download Ready in TorBox. HB14 alone is published; all 38 Hollowed IDs remain `locked:false`, and the other 37 remain reserved and blocked.
- **Out of scope:** Nuvio validation or compatibility-locking of HB14, publishing HB15 or any later/optional/EX entry, publishing Concentrated CB52+, TorBox access by this project, a full-catalog import, deployment, and changes beyond this validation record.
...
The spreadsheet v3 notes remain preserved as unresolved source claims. Usable v3 media and a sufficiently authoritative mapping are not currently available to this project, so future v3 adoption is not automatic and requires a fresh audit plus an explicit owner-reviewed migration decision. The 38 ignored local MP4s are acquisition payloads, not authoritative editorial sources. Their deterministic trackerless torrents now exist only in the ignored local workspace, were independently reproduced byte-for-byte, and verified across all 33,764 pieces. The committed `evidence/acquisition/hollowed-current-raw.json` manifest and 38 `evidence/media/hb_*.json` records preserve the observed English AAC audio and zero embedded subtitle tracks; `(sub)` in a filename is not subtitle-stream evidence.

HB34 and HB36 retain their fresh measured container durations as technical provenance despite material differences from normalized editorial runtime. Editorial runtime remains authoritative for Stremio presentation. The project owner confirmed the 38/38 qBittorrent seeding and TorBox Download Ready availability gate without exposing private integration material. HB14 alone is published through the ordinary runtime path at S3E17/global index 54 and remains `locked:false`; the other 37 Hollowed entries remain blocked, unpublished, reserved, and unlocked, beginning with HB15. Nuvio playback has not yet been validated for HB14, and no permanent public availability claim is made.
...
For the explicit 53-entry locked prefix, `locked:true` means the addon-facing identity, order, torrent, and stream compatibility contract is accepted as the stable baseline; future changes require deliberate revalidation. The first 37 entries retain their preboundary history. The CB36–CB51 extension is supported by deterministic editorial/projection derivation, verified-media evidence, exact deterministic torrents and 4,771/4,771 piece verification, 16/16 user-side seed/cache confirmation, and representative beginning/middle/end real-device sampling. Publication and compatibility lock remain separate decisions: publication now contains 54 entries while the lock remains exactly the first 53.
...
Automated tests protect the 54-entry deterministic publication output ending `cb_51`, `hb_14`; require the explicit 53-ID compatibility-locked baseline to remain an ordered prefix; preserve all existing Concentrated stream/torrent identities; keep HB14 unlocked; and keep HB15, optional, and EX content unpublished. Tests do not reproduce the user-confirmed availability gate or manual Nuvio/TorBox experiments. The 2026-08-20 records preserve both prior terminal focus-trap observations as accepted non-blocking limitations with unresolved attribution; HB14 still requires separate Nuvio validation before any compatibility lock decision.
