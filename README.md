...
- **Compatibility-locked batch:** the first 37 published IDs, from `cb_1` through `cb_35` including `cb_27p5`, followed by guided CB35.5 / `cb_0p0`, remain `published` and `locked:true`. Publication does not automatically expand this explicit lock set.
...
- **Arrancar publication state:** CB36–CB51 have separate verified local acquisition and media evidence. All 16 trackerless torrents were independently recreated byte-for-byte and verified across all 4,771 pieces. The project owner confirmed the exact 16 torrents are seeded and acquired/cached in the user's TorBox environment. They are now `published` for compatibility testing and remain `locked:false`.
- **Arrancar testing still pending:** no CB36–CB51 Nuvio playback test has occurred. The seed/cache confirmation does not establish permanent public availability, future cache persistence, or arbitrary third-party availability.
- **Out of scope:** publishing Hollowed 14 or anything later, publishing Concentrated CB52+, locking CB36–CB51, TorBox access by this project, a full-catalog import, deployment, and changes beyond this controlled publication.
...
The source spreadsheet identity remains `concentrated:0.0`, displayed identifier `0.0`, title `the rotator / the sand`. The separate Watch Guide identity remains guided `35.5`. The project-owner resolution `editorial-resolution:concentrated-35.5-to-0.0` links those facts for the default watch order without rewriting either source. The record keeps permanent ID `cb_0p0`, is published and compatibility-locked as S2E28/global index 37. CB36 begins the newly published testing batch at S3E1/global index 38 and remains unlocked.
...
The project owner subsequently confirmed all 16 exact torrents at 100% and seeding and confirmed all 16 were acquired/cached in the user's TorBox environment. That precondition authorized controlled publication: CB36–CB51 are now present in production meta, streams, and provenance as published but `locked:false`. The compatibility-locked prefix remains the first 37 entries through guided CB35.5 / `cb_0p0`. The torrent artifacts live only under the ignored local `sources/torrents/concentrated-arrancar/` workspace. Nuvio playback has not yet been tested for this batch, and no permanent availability or future-cache claim is made.
...
Automated tests protect the 53-entry deterministic publication output, require the explicit 37-ID compatibility-locked baseline to remain an ordered prefix, preserve the permanent CB1–CB3 fixtures, keep published CB36–CB51 unlocked, and keep every reserved ID unlocked. They do not reproduce the user-confirmed TorBox cache preconditions or the manual Nuvio/TorBox experiments. The 2026-08-20 manual record supplies the corrected CB35→CB35.5 terminal validation and preserves the accepted focus-trap limitation; CB36–CB51 still require Nuvio testing, and Hollowed 14 remains unpublished.
