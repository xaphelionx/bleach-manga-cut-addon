...
- **Compatibility-locked prefix:** all 54 published IDs through `hb_14` are `locked:true`. HB14 was locked only after its separate real-device validation; publication does not grant a compatibility lock automatically.
...
For the explicit 54-entry locked prefix, `locked:true` means the addon-facing identity, order, torrent, and stream compatibility contract is accepted as the stable baseline; future changes require deliberate revalidation. The first 37 entries retain their preboundary history. The CB36–CB51 extension is supported by deterministic editorial/projection derivation, verified-media evidence, exact deterministic torrents and 4,771/4,771 piece verification, 16/16 user-side seed/cache confirmation, and representative beginning/middle/end real-device sampling. HB14 adds its separately validated compatibility proof. Publication and compatibility lock remain separate decisions; the HB14 lock does not validate or authorize HB15 or the remaining Hollowed records.
...
Automated tests protect the 54-entry deterministic publication and compatibility-locked output ending `cb_51`, `hb_14`; preserve all existing Concentrated stream/torrent identities; preserve HB14's raw technical evidence and corrected user-facing presentation; and keep HB15, optional, and EX content unpublished. Tests do not reproduce the user-confirmed availability gate or manual Nuvio/TorBox experiments. The dated records preserve all terminal focus-trap observations as accepted non-blocking limitations with unresolved attribution.
