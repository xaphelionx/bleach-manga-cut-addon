...
This Stremio addon publishes the current 53-entry default Concentrated Bleach prefix from CB1 through CB51, including CB27.5 and guided CB35.5. All 53 published entries are compatibility-locked as the accepted batch baseline after deterministic technical validation and representative real-device sampling. The first 37 entries retain their prior preboundary validation history; CB36–CB51 extend the baseline through the documented representative Arrancar validation against Nuvio 0.8.4-beta with its native TorBox integration. **Concentrated Bleach 01 — Death and Strawberry**, **Concentrated Bleach 02 — Starter**, and **Concentrated Bleach 03 — The Pink-Cheeked Cockatiel** retain their stronger individually established historical fixtures. The batch lock does not imply that every episode was manually played or that every client UI behavior is bug-free. This is not a full catalog and is not deployed.

Current publication and validation state:

- **Published:** 53 default Concentrated entries: CB1–CB51, including CB27.5 and guided CB35.5. Guided CB35.5 is spreadsheet record `concentrated:0.0`, internal ID `cb_0p0`, and the final Season 2 episode at S2E28; CB36–CB51 occupy S3E1–S3E16.
- **Compatibility-locked batch:** all 53 published IDs through `cb_51` are now `published` and `locked:true`. The explicit lock decision remains separate from publication and does not automatically expand with future publication.
...
- **Canonical prefix stop:** Hollowed 14 (`hb_14`) is the first unpublished default entry at S3E17/global index 54. Concentrated CB52+ are reserved source identities, not the next selected Watch Guide entries.
...
- **Hollowed technical evidence:** the owner-selected 38-entry current raw membership now has a separate deterministic acquisition manifest and 38 verified-media records. All 38 local trackerless torrents were independently reproduced byte-for-byte and all 33,764 pieces verified against the selected MP4 payloads. Every Hollowed entry remains unpublished, reserved, and `locked:false`.
- **Out of scope:** publishing Hollowed 14 or anything later, publishing Concentrated CB52+, TorBox access by this project, a full-catalog import, deployment, and changes beyond this validation record.
...
HB34 and HB36 retain their fresh measured container durations as technical provenance despite material differences from normalized editorial runtime. Editorial runtime remains authoritative for future Stremio presentation. All 38 Hollowed IDs remain blocked, unpublished, reserved, and `locked:false`; `hb_14` remains the first unpublished default entry. User-side qBittorrent/TorBox availability and Nuvio playback have not been established for this batch, `hb_` runtime serving is not enabled, and no permanent public availability claim is made.
...
Inspection observed one HEVC video and one English AAC LC stereo audio track per file, with no embedded subtitle streams, attachments, or warnings. The filename marker `(sub)` is retained as payload identity but does not create subtitle evidence. HB34 measured 2074.864458 seconds versus the 2133-second editorial runtime, while HB36 measured 2657.529875 seconds versus the 2568-second editorial runtime; those deltas remain technical provenance and do not replace normalized editorial runtime. The local torrents remain ignored, and no user-side seeding/cache precondition, Nuvio playback validation, runtime-serving support, publication, or future-v3 adoption is claimed.
...
Automated tests protect the 53-entry deterministic publication output, require the explicit 53-ID compatibility-locked baseline to remain an ordered prefix, preserve the permanent CB1–CB3 fixtures, and keep every reserved ID unlocked. The architecture still permits a future published prefix to extend beyond the explicit lock set without automatically locking new entries. Tests do not reproduce user-confirmed TorBox cache preconditions or manual Nuvio/TorBox experiments. The 2026-08-20 records preserve both terminal focus-trap observations as accepted non-blocking limitations with unresolved attribution; Hollowed 14 remains unpublished.
