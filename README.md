...
This Stremio addon publishes the current 91-entry default prefix: 53 Concentrated Bleach entries from CB1 through CB51, including CB27.5 and guided CB35.5, followed by the complete 38-entry owner-selected Hollowed current-raw span from HB14 through HB50 with HB0.8 in its established position. The compatibility-locked prefix remains exactly 54 entries through HB14; the 37 newly published Hollowed entries remain unlocked pending representative real-device testing. The first 37 entries retain their prior preboundary validation history; CB36–CB51 extend the baseline through the documented representative Arrancar validation against Nuvio 0.8.4-beta with its native TorBox integration; and HB14 has its own completed real-device validation. **Concentrated Bleach 01 — Death and Strawberry**, **Concentrated Bleach 02 — Starter**, and **Concentrated Bleach 03 — The Pink-Cheeked Cockatiel** retain their stronger individually established historical fixtures. Publication does not imply compatibility validation or permanent availability. This is not a full catalog and is not deployed.
...
- **Compatibility-locked prefix:** exactly 54 IDs through `hb_14` are `locked:true`. The later 37 Hollowed IDs are `published` and `locked:false`; publication does not grant a compatibility lock automatically.
...
Automated tests protect the 91-entry deterministic publication output ending `hb_48`, `hb_49`, `hb_50`; retain the exact 54-ID compatibility-locked prefix through `hb_14`; preserve all existing Concentrated and HB14 stream/torrent identities; derive the new 37 Hollowed streams from committed evidence; omit unvalidated Hollowed semantic language tokens; and keep Chipped, HB11.5, EX, Concentrated CB52+, and Hollowed 51–62 unpublished. Tests do not reproduce the user-confirmed availability gate or manual Nuvio/TorBox experiments. The dated records preserve all terminal focus-trap observations as accepted non-blocking limitations with unresolved attribution.
